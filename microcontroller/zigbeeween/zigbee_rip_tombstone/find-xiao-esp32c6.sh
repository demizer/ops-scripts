#!/bin/bash
# Find Xiao ESP32-C6 device using lsusb output and sysfs

set -u

# Define expected USB device ID for Espressif USB JTAG/serial debug unit
# Xiao ESP32-C6 uses the same Espressif USB JTAG adapter as other ESP32-C6 boards
ESPRESSIF_VID="303a"
ESPRESSIF_PID="1001"

# Check if device exists using lsusb
if ! lsusb | grep -q "$ESPRESSIF_VID:$ESPRESSIF_PID"; then
    echo "ERROR: Xiao ESP32-C6 device not found (ID $ESPRESSIF_VID:$ESPRESSIF_PID)" >&2
    exit 1
fi
