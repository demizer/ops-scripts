...
# Set up PIR motion sensor on pin 18
pir_pin = DigitalInOut(board.IO18)
pir_pin.direction = Direction.INPUT
# pir_pin.pull = Pull.UP
...
while True:
    # Read PIR sensor
    current_motion = pir_pin.value

    # Update NeoPixel based on motion detection
    if current_motion:
    ...
    # LOW BATTERY CHECK - write end file every 1 second when <5%
    if battery_percent < LOW_BATTERY_THRESHOLD and not usb_connected:
        if current_time - last_update_time >= 1.0:  # Every 1 second
    ...
    time.sleep(0.1)  # Check PIR sensor 10 times per second
